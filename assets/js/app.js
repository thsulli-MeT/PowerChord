...
const KBD_KEYS = ["A","S","D","F","G","H","J","K"];
const PAD_COLORS = [
...
function padHoldStart(padIndex, pointerId){
  ...
  const inst = armed ? armed.instrument : (soundSel ? soundSel.value : 'soft_piano');
  const strm = armed ? armed.strum : false;
  const rvs = armed ? (armed.rev ?? 0.22) : 0.22;
  ...
  if (isRecording && isPlaying && armed){
    const b = nowBeats();
    const q = quantizeBeat(b, 0.25) % loopBeats();
    const id = nextEventId++;
    armed.events.push({ id, tBeats: q, padIndex, dBeats: 1.0 });
    activeHolds.get(pointerId).rec = { id, trackId: armed.id, startBeat: q };
    updateLoopBadge();
    renderTracks();
  }
}
...
if (panicBtn){
  panicBtn.addEventListener("click", ()=>{
    ...
    if (master && ac){
      ...
      setTimeout(()=>{ if(master) master.gain.value = 0.0001; }, 50);
    }
  });
}
